var LogContrast = {
    ...
    contrast_function: function(x) {
        var span = 255;
        var xmin = this.blackValue;
        var xmax = this.whiteValue;
        var ymin = this.blackIntensity;
        var ymax = this.whiteIntensity;
    ...
    apply_contrast: function() {
        ...
        $("img.contrast-tile", viewer).mousemove(function(event) {
            IIPhistogram.magnify(event, LogContrast.curview);
        });
    ...
    new: function() {
        ...
        .append("path")
            .datum(d3.range(256).map(function(i) { 
                if (i <= LogContrast.blackValue) {
                    return [i, LogContrast.blackIntensity];
                } else if (i >= LogContrast.whiteValue) {
                    return [i, LogContrast.whiteIntensity];
                } else {
                    return [i, LogContrast.contrast_function(i)]; 
                }
            }))
            .attr("class", "line")
            .attr("d", this.line);
